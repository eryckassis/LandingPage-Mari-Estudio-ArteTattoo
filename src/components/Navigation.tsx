...
import { Menu, X, MessageCircle, User, Image, Phone, Home, Info } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-40 transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-80 bg-gradient-to-b from-black via-purple-950/50 to-black backdrop-blur-xl border-l border-purple-400/30 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
